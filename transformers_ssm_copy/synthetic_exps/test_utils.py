...
def evaluation(args, model, tokenizer, TO_TOKEN):
    ...
    print("\n")        
    print(f"\nFinal Eval results (avg over lengths): acc_str = {np.mean(str_acc_mean_list) * 100:.2f}%, acc_char = {np.mean(char_accuracy_list) * 100:.2f}%")
    ...
    with open(csv_path, mode='a', newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["model", "steps", "hidden_size", "layers", "acc_str", "acc_char"])
        writer.writerow([
            args.model,
            args.steps,
            args.hidden_size,
            args.layers,
            round(np.mean(str_acc_mean_list) * 100, 2),
            round(np.mean(char_accuracy_list) * 100, 2)
        ])
